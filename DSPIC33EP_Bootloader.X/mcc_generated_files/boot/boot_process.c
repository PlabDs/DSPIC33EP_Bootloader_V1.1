...
static enum BOOT_COMMAND_RESULT EraseFlash(void)
{
    ...
    // --- YAZILIMSAL KORUMA BA?LANGICI ---
    // E?er silinmek istenen adres 0x2000'den küçükse (Bootloader bölgesi)
    if (pCommand->address < 0x2000)
    {
        response.success = BAD_ADDRESS; // Hata kodunu ayarla
        // Bilgisayara "Bu bölgeyi silemezsin" de
        BOOT_COM_Write((uint8_t*) & response, sizeof (struct RESPONSE_TYPE_0) / sizeof (uint8_t));
        
        // ??lemi burada kes, silme fonksiyonuna (BOOT_BlockErase) gitme
        return BOOT_COMMAND_ERROR;
    }
    // --- YAZILIMSAL KORUMA B?T??? ---
...
static enum BOOT_COMMAND_RESULT WriteFlash(void)
{
    ...
    // --- KORUMA EKLEMES? BA?LANGIÇ ---
    if (pCommand->address < BOOTLOADER_END_ADDRESS)
    {
        response.success = BAD_ADDRESS; 
        BOOT_COM_Write((uint8_t*) & response, sizeof (struct RESPONSE_TYPE_0) / sizeof (uint8_t));
        return BOOT_COMMAND_ERROR;
